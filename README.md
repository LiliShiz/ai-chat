...
Нужен Node ≥ 20.12 — `.env` грузится через `process.loadEnvFile`,
который появился именно в этой версии. Проверялось на Node 22 (CI) и 23.
...
- Подсветка собрана на шести языках вместо сорока. Готовый
  `rehype-highlight` всегда тянет `common`-набор highlight.js, и
  убрать его опцией нельзя — плагин мержит `languages` поверх, а не
  вместо. Поэтому здесь свой плагин на тридцать строк поверх
  `lowlight` (`web/src/lib/highlight.ts`): ленивый чанк похудел с
  337 до 213 КБ (102 → 67 КБ gzip).
...
69 тестов, Vitest. Они покрывают не «рендерится без падения», а то,
что реально ломается — края, ради которых это задание и написано.

**Сервер (30).** Разбор SSE: кадр, разорванный между сетевыми
...
**Фронт (39).** Чтение потока: сборка текста из кусков, не совпадающих
...
| Кольцо фокуса у кнопки «К последнему» | E2E-тест про фокус на каждой кнопке |
...
### E2E: 21 сценарий в настоящем браузере

```bash
npm run e2e
```

Стенд Playwright поднимает сам: поддельный апстрим, прокси с
заниженными до 4 секунд таймаутами и Vite. Живая модель тут не годится
по той же причине, по которой появился мок: 429 и таймаут на ней
случаются когда повезёт, а тест обязан падать по делу.

Что проверяется в браузере, а не в jsdom:

- **Стриминг именно как стриминг.** Длина текста снимается дважды с
  паузой, и вторая должна быть больше первой. Готовый ответ так не
  отличить от мгновенного — а здесь отличается.
- **Esc и кнопка «Стоп»** обрывают генерацию, частичный текст
  остаётся, появляется пометка, интерфейс принимает следующий вопрос.
- **429** показывает заголовок, текст и живой отсчёт по `Retry-After`;
  пустой пузырь из ленты исчезает, вопрос остаётся.
- **Обрыв посреди потока** сохраняет написанное и предлагает повтор.
- **Молчащий апстрим** ловится сторожевым таймером — это тот самый
  «живой таймаут», который раньше был проверен только на фальшивых
  таймерах.
- **Настоящий offline.** `context.setOffline(true)` выключает сеть на
  уровне браузера: проверяется и состояние, и то, что после
  возвращения сети повтор проходит.
- **Ключ не утекает.** Пишутся все запросы страницы; после полного
  обмена список обращений за пределы `localhost:5173` обязан быть
  пустым, и ни на одном не должно быть заголовка авторизации.
- **История** переживает перезагрузку, «Очистить» стирает и её.
- **Тема** переключается, переживает F5 и возвращается к системной.
- **Клавиатура:** доходим до поля табом, Enter отправляет,
  Shift+Enter переносит строку, Esc останавливает.
- **Фокус:** кольцо есть при Tab и нет при клике мышью — проверяется
  на кнопке, потому что текстовые поля по спецификации матчат
  `:focus-visible` всегда.
- **Семантика:** один `main`, один `header`, один `footer`, лента —
  `ol > li`, а не набор блоков.
- **Живой регион** висит ровно на одном ответе — том, который сейчас
  пишется.
- **375px:** горизонтального переполнения нет, тач-таргеты от 44px.

### CI

`.github/workflows/ci.yml` — две задачи.

**Линт, типы, тесты, сборка, прод-режим.** Последний шаг поднимает
`npm start` поверх поддельного апстрима и проверяет: `/api/health`;
что по `/` приходит **собранный** фронт (ищется ссылка на хешированный
ассет, а не строка, которая есть и в исходном `index.html`); что в
скачанном бандле нет ключа; что `/api/chat` **действительно стримит**;
что неизвестный `/api`-путь отдаёт JSON-404, а не HTML; и что
валидация работает. Каждая из этих проверок добавлена после того, как
предыдущая версия шага оказалась зелёной при сломанном коде — см.
ИИ-лог, пункты 12 и 24.

**E2E в браузере.** При падении отчёт Playwright с трассировкой и
видео сохраняется артефактом.
...
- **Реальное устройство.** Мобильная вёрстка проверена в браузере на
  375 и 390px, в том числе автоматически (`e2e/a11y.spec.ts` меряет
  горизонтальное переполнение и высоту тач-таргетов), но на живом
  телефоне я её не открывал. Единственный пункт из этого списка,
...
ТЗ просило уложиться в 2–6 часов чистого времени. По истории коммитов
активной разработки вышло **около 5 часов**, разбитых на девять
подходов за два дня.

Считал так: коммиты сгруппированы в сессии с разрывом больше 45 минут,
к каждой сессии добавлено по 12 минут на работу до первого коммита.

| Когда | Подходов | Чистого времени |
|---|---|---|
| 24.09, первый заход | 7 | ≈ 4,4 ч |
| 25.09, доработки по ревью | 2 | ≈ 0,4 ч |
| **Итого** | **9** | **≈ 4,9 ч** |

Календарно между первым и последним коммитом прошло 29 часов —
остальное это паузы.
...
| | |
|---|---|
| Кода (TS/TSX/CSS/HTML) | 6 092 строки |
| Из них тесты | 1 893 строки |
| Коммитов | 30 без учёта merge |
| Слитых PR | 14 |
| Тестов | 69 юнит + 21 браузерный |

Примерно половина этого времени ушла не на то, чтобы написать
работающий чат — он был готов к концу первого подхода, — а на то,
чтобы убедиться, что он действительно работает. Разница между этими
двумя состояниями и описана ниже.
...
│   └── a11y.spec.ts         клавиатура, фокус, семантика, 375px
