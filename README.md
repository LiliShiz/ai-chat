...
| `npm test` | Тесты сервера и фронта (48 штук) |
...
48 тестов, Vitest. Они покрывают не «рендерится без падения», а то,
что реально ломается — края, ради которых это задание и написано.

**Сервер (23).** Разбор SSE: кадр, разорванный между сетевыми
...
**Фронт (25).** Чтение потока: сборка текста из кусков, не совпадающих
...
Сами тесты я проверил мутацией: сломал флаг «остановлено» в
`useChat` — тест упал на нужной строке. Набор, который проходит при
сломанном коде, хуже, чем его отсутствие.

CI (`.github/workflows/ci.yml`) гоняет `typecheck`, тесты и сборку на
каждый push и PR.
