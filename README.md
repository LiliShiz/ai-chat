...
| `npm test` | Юнит-тесты сервера и фронта (59 штук) |
| `npm run e2e` | Браузерные тесты, 19 сценариев (стенд поднимается сам) |
...
59 тестов, Vitest. Они покрывают не «рендерится без падения», а то,
что реально ломается — края, ради которых это задание и написано.

**Сервер (28).** Разбор SSE: кадр, разорванный между сетевыми
...
### E2E: 19 сценариев в настоящем браузере
...
**Типы, тесты, сборка, прод-режим.** Последний шаг поднимает `npm start`
поверх поддельного апстрима и проверяет четыре вещи: `/api/health`,
что по `/` отдаётся собранный фронт, что `/api/chat` **действительно
стримит** (в ответе есть `"type":"delta"`), и что валидация работает.
Появился он не просто так — см. ИИ-лог, пункт 12.
